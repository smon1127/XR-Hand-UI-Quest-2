...
public class InverseKinematics : MonoBehaviour {
	...
	void Start () {
		...
		if (target == null)
		{
			target = GameObject.Find("LeftHandAnchor").transform;
		} else
        {
			Debug.Log("LeftHandAnchor not available");
        }		
		

	}
	//IEnumerator LeftHandDeactivate(float sec)
	//{
	...
	void LateUpdate () {
		//StartCoroutine(LeftHandDeactivate(2));
