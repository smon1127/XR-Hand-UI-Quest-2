...
namespace Microsoft.MixedReality.Toolkit.Utilities.Solvers
{
    ...
    public class orbitalViewAnchor : Solver
    {
        ...
        {                      
            if (isFollow)
            {
                ...
                gameObject.GetComponent<Solver>().MoveLerpTime = 0.44f;
                Vector3 anchorOffset = transform.position;
                anchorOffset.y -= LocalOffsetCustom.y;
                anchorOffset.z -= LocalOffsetCustom.z;
                anchorPivot.position = anchorOffset;
                Debug.Log("a: " + anchorPivot.position);
